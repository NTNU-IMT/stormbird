...
This type of linearized solver is available as an option in the Stormbird library, with an extentions that empirically correct for viscous effects on the lift in a very simple manner. The extension is a requirement for it to be useful for wind propulison devices, which experiences large viscous effects on the lift.

However, the fundamental assumptions behind a linearized equation system is technically not correct when either the lift is very large, or when the lifting surface is operating close to stall. To potentially solve these situations more accuratly, Stormbird also implement a slower, but also simple and robust approach based on *dampened iterations*. The solver is largely inspired by the methods described in Anderson (2005), chapter 5.4. This makes it possible to simulate wings operating close to, and above, stall, and the shape of the sectional lift can be arbitrary.

Which solver to choose depends on the specific requirements of the case. More details is given in the [solver chapter](solver.md)
