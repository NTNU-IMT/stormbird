...
// WGSL resolves module-scope items regardless of textual order, so this doesn't need those
// declared before it in the assembled source, only present somewhere in it.
//
// No ghost cells: `current`/`rhs` are sized to the INTERIOR grid only. Boundary conditions are
// folded directly into the stencil instead of being read from a padded array — see
// `axis_off_diagonal`'s doc comment for the 4th order substitution rule. This is an *explicit*
// substitution using the already-known `current` values (matching the lagged ghost cells a
// separate ghost-cell pass would have produced from the *previous* Jacobi sweep), not an implicit
// one — it must be added to `off_diag`, not folded into the diagonal coefficient, since it isn't
// solving for the new value self-referentially.
...
fn jacobi_relaxed_update(idx: u32, ii: u32, ji: u32, ki: u32) -> f32 {
    ...
    return (1.0 - JACOBI_WEIGHT) * current[idx] + JACOBI_WEIGHT * jacobi_update;
}
