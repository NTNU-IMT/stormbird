...
/// stencil (jacobi, restrict, coarse-solve). Boundary conditions don't depend on grid resolution,
/// so this only needs to be baked in once per solver, not per level.
