...
impl EffectiveAngleOfAttackControllerBuilder {
    ...
    pub fn build(&self) -> EffectiveAngleOfAttackController {
        ...
        let current_wing_angles = vec![0.0; nr_of_wings];
...
impl EffectiveAngleOfAttackController {
    ...
    pub fn update(&mut self, time_step: f64, simulation_result: &SimulationResult) -> Option<ControllerOutput> {
        ...
        if first_time_step || time_to_update {
            let new_local_wing_angles = self.compute_new_local_wing_angles(time_step);

            let change_necessary = self.set_new_local_wing_angles_and_check_for_change(
    ...
    pub fn set_new_local_wing_angles_and_check_for_change(&mut self, new_local_wing_angels: &[f64]) -> bool {
        let mut change_necessary = false;
        
        for i in 0..self.nr_of_wings() {
            let angle_error = new_local_wing_angels[i] - self.current_wing_angles[i];
        ...
        if change_necessary {
            self.current_wing_angles = new_local_wing_angels.to_vec();
        }

        change_necessary
    }

    pub fn compute_new_local_wing_angles(&self, time_step: f64) -> Vec<f64> {
        ...
        for i in 0..nr_of_wings {
            let max_rotational_speed: f64 = if self.target_angles_of_attack[i] == 0.0 {
            ...
            new_local_wing_angles[i] = self.current_wing_angles[i] + raw_angle_change;

            while new_local_wing_angles[i] > std::f64::consts::PI {
                new_local_wing_angles[i] -= 2.0 * std::f64::consts::PI;
            }
            while new_local_wing_angles[i] < -std::f64::consts::PI {
                new_local_wing_angles[i] += 2.0 * std::f64::consts::PI;
            }
